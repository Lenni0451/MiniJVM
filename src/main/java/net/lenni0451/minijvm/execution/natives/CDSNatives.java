...
public class CDSNatives implements Consumer<ExecutionManager> {
    ...
    public void accept(ExecutionManager manager) {
        manager.registerMethodExecutor("jdk/internal/misc/CDS.isDumpingClassList0()Z", (executionManager, executionContext, currentClass, currentMethod, instance, arguments) -> {
            return returnValue(new StackInt(0));
        });
        manager.registerMethodExecutor("jdk/internal/misc/CDS.isSharingEnabled0()Z", (executionManager, executionContext, currentClass, currentMethod, instance, arguments) -> {
            return returnValue(new StackInt(0));
        });
        manager.registerMethodExecutor("jdk/internal/misc/CDS.isDumpingArchive0()Z", (executionManager, executionContext, currentClass, currentMethod, instance, arguments) -> {
            return returnValue(new StackInt(0));
        });
        ...
        manager.registerMethodExecutor("jdk/internal/misc/CDS.getRandomSeedForDumping()J", (executionManager, executionContext, currentClass, currentMethod, instance, arguments) -> {
